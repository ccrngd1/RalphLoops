...
class Config(BaseModel):
    ...
    # Kiro CLI (R15.3). ``default_model_id`` is passed as ``--model <id>``
    # on every Kiro CLI invocation (persona execution, persona_review,
...
#
# The Token Accountant records one ``LlmCallRecord`` per Kiro CLI / LLM call
...
class LlmCallRecord(BaseModel):
    ...
    because Kiro CLI may omit the token envelope (R12.6) and because pricing
...
class KiroInvocationLog(BaseModel):
    """Summary of the Kiro CLI subprocess invocation for the iteration (R11.3)."""
...
class PersonaSelection(BaseModel):
    ...
    ``token_usage`` is populated when Kiro CLI reports tokens (R12.1).
    """
...
class ContextWindow(BaseModel):
    """The composed prompt handed to Kiro CLI (R6.1, R6.7).
...
class KiroInvocationResult(BaseModel):
    """Result of a single Kiro CLI invocation (R6.1, R11.2, R12.1, R12.6).
    ...
    Kiro CLI stdout did not contain a parseable token envelope (R12.6).

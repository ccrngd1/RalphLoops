...
def task_and_exception(draw):
    ...
    the project-specific ``KiroInvocationTimeout``, and the standard
    ...
    if exc_choice == "runtime":
        ...
        # ``KiroInvocationTimeout`` is a plain ``Exception`` subclass; it
        # accepts any positional message string. Fall back to a literal
        # ``"timeout"`` so ``str(exc)`` is never empty.
        exc = KiroInvocationTimeout(msg or "timeout")
    else:

...
single :class:`KiroInvocationResult` (or raised exception).
"""
...
def _make_invocation_result_for_outcome(
    ...
    Optional[KiroInvocationResult], Optional[BaseException], Optional[str]
]:
    """Build the mock ``KiroInvoker.invoke`` return value for a given outcome.
    ...
    if outcome == "timeout":
        return (None, KiroInvocationTimeout("took too long"), None)

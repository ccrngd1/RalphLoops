...
from ralph_loop.kiro import KiroInvocationTimeout
...
# ---------------------------------------------------------------------------
# Synthetic CheckResult and KiroInvocationTimeout handling
# ---------------------------------------------------------------------------
...
class TestSyntheticCheckResult:
    ...
    def test_timeout_sets_timed_out_true_on_synthetic(
        ...
        # When the underlying exception is ``KiroInvocationTimeout``, the
        ...
        exc = KiroInvocationTimeout("ran over the per-call budget")
        ...
        assert synthetic.name == "kiro_invocation"
...
class _ScriptedKiroInvoker:
    """Script-driven stand-in for :class:`ralph_loop.kiro.KiroInvoker`.
    ...
    KiroInvocationResult"). Once a task's script is exhausted, all
    ...
    used by ``_StubKiroInvoker`` in :mod:`tests.test_cli`.
    """
    ...
    async def invoke(
        ...
        *,
        ...
        from ralph_loop.models import KiroInvocationResult
...
async def _drive_run_loop(
    *,
    ...
    monkeypatch.setattr(cli_mod, "KiroInvoker", lambda *a, **kw: invoker)
...
class TestProperty24LoopExitCode:
    ...
    async def test_scenario_c_retry_cap_exhaustion_returns_exit_blocked(
        ...
        invoker = _ScriptedKiroInvoker(raise_every_call_for={"A"})
        validator = _PassingValidator()
        git = _NoopGitManager(enabled=False, cwd=project)

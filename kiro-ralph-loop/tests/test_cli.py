...
class _StubKiroInvoker:
    ...
    invocation returns a passing :class:`KiroInvocationResult`.
    """
...
async def test_graceful_invocation_error_continues_loop(
    ...
    The stub KiroInvoker raises ``RuntimeError("chunk exceed the limit")``
    on A's first invocation and returns a passing KiroInvocationResult
    ...
    # The loop constructs ``KiroInvoker(kiro_cli_command=...)`` inline
    ...
    # re-constructions (the loop may build multiple ``KiroInvoker``
    # instances across its component wiring).
    monkeypatch.setattr(
        cli_mod, "KiroInvoker", lambda *args, **kwargs: stub_invoker
    )
    monkeypatch.setattr(

"""Integration tests for :class:`ralph_loop.kiro.KiroInvoker` (Task 13.2).

These tests stub the real ``kiro-cli`` binary with a small Python script
...
envelope line, so we can assert that the :class:`KiroInvoker`:
...
def _write_stub(
    ...
    *,
    ...
    """Write a tiny Python script mimicking the Kiro CLI contract.
    ...
    The arguments received (``chat --no-interactive``) are also echoed so
    we can assert the invoker invoked the right subcommand.
    """

    stub_path = tmp_path / "fake_kiro.py"
...
def _invoker_for(stub_path: Path) -> KiroInvoker:
    """Build an invoker whose ``kiro_cli_command`` runs the stub via ``sys.executable``."""
    # Quote both pieces so spaces in paths (common on Windows) survive shlex.
    return KiroInvoker(kiro_cli_command=f'"{sys.executable}" "{stub_path}"')
...
async def test_invoke_captures_stdin_stdout_and_parses_token_envelope(
    ...
    token_line = (
        f'{TOKEN_MARKER} '
        '{"input_tokens": 123, "output_tokens": 45, "model": "m-1"}'
    )
    stub = _write_stub(
        tmp_path,
    ...
    assert "ARGS:chat --no-interactive" in result.stdout
...
async def test_invoke_malformed_token_envelope_is_ignored(
    ...
    """A marker line with invalid JSON does not crash; token_usage stays None (R12.6)."""

    stub = _write_stub(
        tmp_path,
        stdout_lines=[f"{TOKEN_MARKER} not-json-here"],
        exit_code=0,
    )
    invoker = _invoker_for(stub)
...
async def test_invoke_timeout_raises_and_kills_subprocess(
    ...
    """A subprocess exceeding ``timeout_ms`` raises KiroInvocationTimeout."""
    ...
    with pytest.raises(KiroInvocationTimeout):

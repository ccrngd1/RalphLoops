"""Configurable stub of ``kiro-cli chat --no-interactive`` for integration tests (Task 23.1).

This script is invoked by integration tests as the ``kiro_cli_command``
via ``sys.executable`` so no real Kiro CLI binary is required. Behaviour
is controlled through environment variables set by the test before
spawning the :class:`ralph_loop.kiro.KiroInvoker`. The harness supports
the three things the integration suite needs:

* Emitting an arbitrary stdout payload (``RALPH_STUB_STDOUT``).
* Optionally appending a ``RALPH_TOKEN_USAGE:`` envelope line so the
...
def main() -> None:
    ...
    # pipe (the real Kiro CLI also consumes its stdin).
    try:
        sys.stdin.read()
    ...
    if mutate_path and tasks_path:
        try:
            ...
            sys.stderr.write(f"fake_kiro mutate failed: {exc}\n")
    ...
    # Optionally emit a token envelope line so the invoker can parse
    ...
    if _resolve("RALPH_STUB_EMIT_TOKENS", overrides) == "1":
        ...
        sys.stdout.write(f"RALPH_TOKEN_USAGE: {payload}\n")

    # ``persona_review`` verdict helpers. The validator spawns the
    # reviewing persona in its own Kiro CLI session and parses a

...
KiroInvoker, Validator, TaskCreationProcessor, Planner, GitManager,
...
@click.group(
    help="Ralph Loop: domain-agnostic automated iteration wrapper for Kiro CLI."
)
def main() -> None:
...
@click.option(
    ...
    help=(
        ...
        "every Kiro CLI invocation (persona execution, persona_review, "
...
async def _run_loop(config: Config, project_root: Path) -> int:
    ...
    invoker = KiroInvoker(kiro_cli_command=config.kiro_cli_command)
    ...
    while True:
        ...
        # Flip task to in_progress and persist before invoking Kiro CLI
...
@click.option(
    ...
    default=None,
    help=(
        "Override Config.default_model_id for the planner's Kiro CLI "

...
* ``persona_review`` (:func:`_run_persona_review_check`): resolves the
  ...
  R7.6-R7.8), invokes the reviewing persona in a separate Claude Code CLI
...
class PersonaReviewVerdict(BaseModel):
    ...
    The reviewing persona's Claude Code CLI session is instructed to emit this
...
def _build_review_prompt(
    ...
    * Asks for a strict JSON verdict of the form
    ...
    ``tasks.json``. Combined with the separate Claude Code CLI session
...
async def _run_persona_review_check(
    ...
    *,
    ...
    4. Build the review prompt and invoke Claude Code CLI with
    ...
    except ClaudeCodeInvocationTimeout:
        duration_ms = int((time.monotonic() - start) * 1000)
        logger.warning(
    ...
    async def run(
        ...
        * :class:`PersonaReviewCheckConfig` ->
        ...
        Args:
            ...
            log_path: Log file receiving ``persona_review`` Claude Code CLI

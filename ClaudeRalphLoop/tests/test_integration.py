...
(:mod:`tests.support.fake_claude`). They complement the component-level
...
FAKE_KIRO = Path(__file__).parent / "support" / "fake_claude.py"


def _kiro_command() -> str:
    """Return a ``claude_cli_command`` that invokes the fake harness."""
...
class TestFakeKiroHarness:
    """Validates that the ``tests/support/fake_claude.py`` harness itself is
    ...
    :class:`ClaudeCodeInvoker` without a Kiro CLI install.
    ...
    def test_fake_claude_exists_and_is_executable(self) -> None:
        assert FAKE_KIRO.is_file(), f"fake_claude.py missing at {FAKE_KIRO}"
    ...
    def test_emits_token_envelope_when_enabled(self) -> None:
        ...
        # The token line is emitted as plain JSON (no RALPH_TOKEN_USAGE: marker).
        token_line = next(
            line for line in result.stdout.splitlines()
            if line.startswith("{")
        )
        parsed = json.loads(token_line)
    ...
    def test_omits_token_envelope_by_default(self) -> None:
        ...
        # Without RALPH_STUB_EMIT_TOKENS=1 no JSON token line is emitted.
        token_lines = [
            line for line in result.stdout.splitlines()
    ...
    async def test_kiro_invoker_parses_stub_output(
        ...
        """ClaudeCodeInvoker can drive the fake harness and parse its envelope (R6.1, R12.1)."""
        ...
        try:
            invoker = ClaudeCodeInvoker(claude_cli_command=_kiro_command())
        ...
        assert '"input_tokens"' in log_text  # token JSON captured in log
...
class TestPlannerBootstrapIntegration:
    """End-to-end planner bootstrap driving the real :class:`ClaudeCodeInvoker`
    ...
    async def test_planner_admits_three_tasks_and_records_token_usage(
        ...
        # R12.1: the envelope was emitted and captured in the log (plain JSON format).
...
class TestBookWritingSmoke:
    ...
    * the loop's completion rule fires when every task is ``passing``
    ...
    def test_explicit_target_persona_routes_without_llm(
        ...
        invoker = ClaudeCodeInvoker(claude_cli_command=f'"{sys.executable}" -c "import sys;sys.exit(99)"')

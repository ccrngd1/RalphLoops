...
``AsyncMock(spec=ClaudeCodeInvoker)`` so the reviewing persona's "LLM call"
is deterministic.
"""
...
class TestPersonaReviewCheck:
    ...
    async def test_timeout_marks_timed_out(self, tmp_path: Path) -> None:
        ...
        invoker = AsyncMock(spec=ClaudeCodeInvoker)
        invoker.invoke.side_effect = ClaudeCodeInvocationTimeout("timeout")
...
class TestValidatorRun:
    ...
    async def test_stuck_condition_propagates_from_persona_review(
        ...
        validator = Validator(
            invoker=AsyncMock(spec=ClaudeCodeInvoker), registry=registry
        )

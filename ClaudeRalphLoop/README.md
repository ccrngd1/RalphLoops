...
CLI flags on `ralph run` override `max_iterations`, `max_retries_per_task`, `wall_clock_timeout_ms`, and `personas_dir` for a single run (R15.2). `ralph init-tasks` accepts `--personas-dir` for the same reason — useful when you want to point at a shared persona set without copying files into the project.
...
A ready-to-use book-authoring persona set is available in the repository's `../templates/book/personas/` directory (Writer, Reviewer, Editor, FactChecker, Outline, Planner). You can use these in two ways:
...
**Option 2: Reference directly with --personas-dir**
```bash
